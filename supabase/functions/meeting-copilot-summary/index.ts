...
${fullText}`;
      ...
      if (aiRes.status === 429) {
        await logMeetingAI({ userId: user.id, action: 'meeting_copilot_summary', model: 'google/gemini-3-flash-preview', latencyMs: aiLatency, status: 'error', errorMessage: 'rate_limited', metadata: { sessionId } });
        return new Response(JSON.stringify({ error: 'rate_limited' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      if (aiRes.status === 402) {
        await logMeetingAI({ userId: user.id, action: 'meeting_copilot_summary', model: 'google/gemini-3-flash-preview', latencyMs: aiLatency, status: 'error', errorMessage: 'credits_exhausted', metadata: { sessionId } });
        return new Response(JSON.stringify({ error: 'credits_exhausted' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      if (!aiRes.ok) {
        await logMeetingAI({ userId: user.id, action: 'meeting_copilot_summary', model: 'google/gemini-3-flash-preview', latencyMs: aiLatency, status: 'error', errorMessage: `http_${aiRes.status}`, metadata: { sessionId } });
        return new Response(JSON.stringify({ error: 'ai_error' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      const aiData = await aiRes.json();
      await logMeetingAI({ userId: user.id, action: 'meeting_copilot_summary', model: 'google/gemini-3-flash-preview', usage: aiData?.usage, latencyMs: aiLatency, metadata: { sessionId, wordCount } });
        ...
        if (token) {
          ...
          const recapHtml = `
            <div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:680px;margin:0 auto;color:#111">
              ...
              <p style="color:#999;font-size:12px;margin-top:32px">Generated by Nikkore Inbox Meeting Copilot.</p>
            </div>
          `;
