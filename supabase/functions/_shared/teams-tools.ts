// Shared tool implementations for the Nikkore Inbox Teams bot.
...
/**
 * Resolve a Teams sender (AAD object id + email) to an Nikkore Inbox user and a
...
- Footer: "Generated by Nikkore Inbox • Sample data for presentation purposes".
  ...
  try {
    ...
    if (kind === 'docx_document') {
      ...
      const uploaded = await uploadToOneDrive({
        graphToken, folder: 'Nikkore Inbox-Artifacts', filename, content: buf, contentType: mime,
      });
      ...
      return JSON.stringify({
        ...
        message: `Saved Word document "${filename}" to your OneDrive in folder "Nikkore Inbox-Artifacts".`,
      });
    }
    ...
    const uploaded = await uploadToOneDrive({
      graphToken,
      folder: 'Nikkore Inbox-Artifacts',
      filename,
      content,
      contentType: mime,
    });
    ...
    return JSON.stringify({
      ...
      onedrive_url: uploaded.webUrl,
      share_url: uploaded.shareUrl,
      message: `Saved "${filename}" to your OneDrive in folder "Nikkore Inbox-Artifacts". Click the share link to open it.`,
    });
  } catch (e) {
