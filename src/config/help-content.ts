...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    steps: [
      { title: 'Open the Daily Brief', description: 'Sidebar → My Daily Brief. The page recomputes on every visit so you always see the freshest read of your inbox and calendar — no manual refresh required.', route: '/ai-daily-brief', target: '[data-tour="brief-hero"]' },
      ...
      { title: 'Action item card details', description: 'The colored left bar shows urgency. Inside the card, Context explains why the item matters, Do tells you the next action, Why explains the reasoning, and the time estimate helps you plan your day.', target: '[data-tour="brief-action-item"]' },
      { title: 'Priority and carry-over badges', description: 'The HIGH / MEDIUM / LOW badge follows your priority colors. A "Carried over ×N" pill means this item appeared in previous briefs and stays visible until you Mark done or the underlying email/meeting is resolved.', target: '[data-tour="brief-item-badges"]' },
      { title: 'Mark done', description: 'Closes the item permanently. It will not appear in tomorrow\'s brief.', target: '[data-tour="brief-mark-done"]' },
      { title: 'Snooze', description: 'Hides the item until tomorrow\'s brief, where it returns with a fresh carry-over count.', target: '[data-tour="brief-snooze"]' },
      { title: 'Remind me', description: 'Creates a follow-up reminder in your No Reply Tracker so you get pinged if the thread stays silent.', target: '[data-tour="brief-remind"]' },
      { title: 'Schedule', description: 'Proposes time slots from your availability and books a calendar event to handle the item.', target: '[data-tour="brief-schedule-action"]' },
      ...
      { title: 'Send to address', description: 'This is the email address that will receive the scheduled brief. Briefs are always sent from agent@energyforward.com.', target: '[data-tour="brief-schedule-send-to"]' },
      { title: 'Timezone', description: 'The schedule runs in this timezone, so 8:00 AM means 8:00 AM in the timezone selected here.', target: '[data-tour="brief-schedule-timezone"]' },
      { title: 'Schedule presets', description: 'Click Weekdays (Mon–Fri), Every day, Weekends, or Custom to add a recurring delivery. You can add multiple schedules (e.g. a morning brief and an evening recap).', target: '[data-tour="brief-schedule-presets"]' },
      { title: 'Schedule row controls', description: 'The toggle enables/disables that schedule without deleting it. The pencil edits time and days; the trash deletes it. The ACTIVE badge confirms the next run will fire.', target: '[data-tour="brief-schedule-row"]' },
      { title: 'Enable or pause a schedule', description: 'Use this switch to temporarily pause a daily brief schedule without deleting the timing setup.', target: '[data-tour="brief-schedule-toggle"]' },
      { title: 'Edit or delete a schedule', description: 'Use the pencil to change the schedule, or the trash icon to remove it completely.', target: '[data-tour="brief-schedule-edit"]' },
      { title: 'Priority Color Settings', description: 'Change the color used for High / Medium / Low priority items everywhere in the brief (web page, email, and PDF). Click a swatch to pick a custom color, or use Reset to Defaults to go back to red/yellow/green.', target: '[data-tour="brief-priority-colors"]' },
      { title: 'High priority color', description: 'This swatch controls the color for urgent items that need action today.', target: '[data-tour="brief-color-high"]' },
      { title: 'Medium priority color', description: 'This swatch controls the color for important items that can usually be handled this week.', target: '[data-tour="brief-color-medium"]' },
      { title: 'Low priority color', description: 'This swatch controls the color for lower-priority items that can wait.', target: '[data-tour="brief-color-low"]' },
    ],
    outro:
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Your Inputs — Responsibilities field', description: 'A short description of what you actually do day-to-day (approvals, follow-ups, contracts, scheduling, etc.). The AI uses this to decide what kind of replies make sense for you.', target: '[data-tour="settings-resp-field"]' },
      { title: 'Responsibilities — Edit and Regenerate', description: 'Click Edit to type your own responsibilities, or click Regenerate to have the AI rewrite them from your title and company.', target: '[data-tour="settings-resp-regenerate"]' },
      { title: 'Your Inputs — Communication style field', description: 'Tone, length, sign-offs, things to avoid. This is the AI\'s style guide for your voice — e.g. "warm but concise, always end with Best, never use exclamation points".', target: '[data-tour="settings-style-field"]' },
      { title: 'Communication style — Edit and Regenerate', description: 'Edit it directly or click Regenerate for a fresh AI suggestion based on your role.', target: '[data-tour="settings-style-regenerate"]' },
      ...
      { title: 'Use Signature Builder', description: 'Recommended mode. It generates a clean, mobile-friendly HTML signature from the fields below — font, color, phone, logo, and photo.', target: '[data-tour="settings-signature-builder"]' },
      { title: 'Paste Custom Signature', description: 'Use this only if you already have custom signature HTML you want to keep pixel-perfect.', target: '[data-tour="settings-custom-signature"]' },
      ...
      { title: 'Builder — Profile Photo area', description: 'Upload a square headshot (max 2 MB). The preview circle shows which profile image will be used in the signature builder.', target: '[data-tour="settings-photo"]' },
      { title: 'Profile Photo — Upload Photo', description: 'Click Upload Photo to choose a square headshot from your computer.', target: '[data-tour="settings-photo-upload"]' },
      { title: 'Profile Photo — Show in signature toggle', description: 'This toggle controls whether the profile photo actually appears in outgoing email. Profile photo WINS over company logo when both are turned on — headshots take priority on personal replies.', target: '[data-tour="settings-photo-toggle"]' },
      { title: 'Builder — Company Logo area', description: 'Upload your company logo (PNG/JPG, max 2 MB, ~200×50 px works best). The preview box shows which logo will be available for the signature.', target: '[data-tour="settings-logo"]' },
      { title: 'Company Logo — Upload Logo', description: 'Click Upload Logo to replace the default company logo with your own brand asset.', target: '[data-tour="settings-logo-upload"]' },
      { title: 'Company Logo — Show in signature toggle', description: 'Toggle this ON to display the logo. The logo only shows when no profile photo is shown, so use logo-only signatures by leaving Profile Photo off.', target: '[data-tour="settings-logo-toggle"]' },
